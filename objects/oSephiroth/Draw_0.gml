...
if (mid_battle_dialogue_visible) {
    var dialogue_x = clamp(oPlayer.x - 150, 10, room_width - 300 - 10);  // Adjust and clamp position relative to the player
    var dialogue_y = clamp(oPlayer.y - 95, 5, room_height - 50 - 10);  // Adjust and clamp position relative to the player
    var dialogue_width = 300;  // Width of the dialogue box
    var dialogue_height = 50;  // Height of the dialogue box
    var border_padding = 4;
...
if (awaiting_choice) {
    ...
    draw_sprite_stretched(sBox, 0, choice_x + 60, choice_y, 180, 60); // Adjust width and height as needed

    draw_set_font(fnOpenSansPX);
    draw_set_color(c_white);
    draw_text(choice_x + 80, choice_y + 10, "Will you fight Sephiroth?"); // Adjust positioning as needed

    // Highlight the selected choice ("Yes" or "No")
    draw_set_color(selected_choice == 0 ? c_yellow : c_white);
    draw_text(choice_x + 80, choice_y + 40, "Yes");

    draw_set_color(selected_choice == 1 ? c_yellow : c_white);
    draw_text(choice_x + 180, choice_y + 40, "No");

    // Draw the pointer for the selected choice
    var pointer_x = selected_choice == 0 ? choice_x + 85 : choice_x + 185; // Adjust pointer position based on selection
