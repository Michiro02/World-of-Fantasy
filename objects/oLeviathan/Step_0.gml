    ...
    if (!post_battle_dialogue_visible) {
      ...
      //  image_blend = c_red;
        if (image_alpha <= 0) {
            ...
            if (instance_exists(oLeviathan)) {
                instance_deactivate_object(oLeviathan);  // Deactivate the enemy object so it doesn't interfere with the dialogue
            }
        }
    }
}
...
if (player_choice == 0 && escapeDelay > 0 && !dialogue_visible) {
    ...
    if (escapeDelay == 1) {
        // Once the escape delay is over, the player can move
		dialogue_sequence = original_dialogue_sequence;
		peaceful_exit = false;
        global.dialogue_active = false;  // Allow movement after escape delay
        player_choice = -1;  // Reset the choice so player can talk to Leviathan again
    }
}
...
if (post_battle_dialogue_visible) {
    if (keyboard_check_pressed(vk_enter)) {
        ...
        if (post_battle_dialogue_index >= array_length(post_battle_dialogue_sequence)) {
            ...
            // Grant Leviathan skill to the player
            var kenneth = global.party[3];
            array_push(kenneth.actions, global.actionLibrary.leviathan);

            // Mark the boss as defeated
            global.LeviathanDefeated = true;

            instance_destroy();  // Only destroy Leviathan after the battle
        }
    }
}
