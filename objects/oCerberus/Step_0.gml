...
if (dead) {
    ...
    if (image_alpha <= 0) {
            ...
            if (instance_exists(oCerberus)) {
                instance_deactivate_object(oCerberus);  // Deactivate the enemy object so it doesn't interfere with the dialogue
            }
        }
    }
}
...
if (start_battle) {
    ...
    NewEncounter(
        choose(
            [global.enemies.cerberus],
        ),
        sBgNewBoss
    );
    audio_play_sound(snd_Cerberus, 1, true);
